...
- Single repo and deployment (monolith) but code organized into modules/apps
  (`src_package.apps.*`).
- Each app contains a `services/` directory — business logic lives in services,
  views are thin.
...
- Cleaner separation: views handle transport (HTTP), services handle business
  logic.

## Structure highlights

- `src/src_package/importer/services/import_service.py` — orchestrates
  validation, batching, db ops.
